# -*- coding: utf-8 -*-
"""
v18.4 全艇スコア解析アプリ（Pure AI・自信度フィルター搭載版）
"""
...
def rank_all(racers: List[Racer], venue: str) -> Tuple[List[Dict], Optional[float]]:
    ...
    for i, r in enumerate(racers):
        ...
        out.append({"lane": lane, "racer": r, "score": ai_score, "1着率": p1, "2着率": p2, "3着率": p3})
...
# メインUI
# ============================================================
st.set_page_config(page_title="v18.4 勝負レース抽出", layout="wide")
st.title("🚤 v18.4 全艇スコア解析 (Pure AI)")
st.caption("完全データ主導型AI / 🎯 自信度フィルター搭載版")

if not lgb_model:
    st.warning("⚠️ 学習済みのAIモデルがありません。AIのスコアはすべて0で計算されます。")
        ...
        if res:
            ...
            for item in ranked:
                ...
                df_disp.append({
                    ...
                    "階級": racer.cls_val, "全国勝率": racer.n_win, "モータ": racer.m_2ren
                })
            ...
            st.write(f"**標準(4点)**: {', '.join(make_bets(ranked, 'standard'))}")
        else:
...
with tab2:
    ...
    confidence_filter = st.slider(
        "【見送りライン】トップ艇の1着率が何%以上なら投票するか（0で全レース投票）", 
    ...
    if st.button("📊 バックテスト実行", type="primary", use_container_width=True):
        ...
        def analyze_race(d, j, r):
            ...
            if not has_result:
                ...
                actual_result = "結果待ち"
                hit_amount = 0
                if not is_confident: actual_result = "見送り推奨"
            else:
                if not is_confident:
                    hit_str = "ー"
                    payoff_disp = f"({payoff})" # 実際の結果を参考表示
                    hit_amount = 0
                    actual_result = "見送り"
                else:
            ...
            return {
                ...
                "トップ勝率": f"{top_prob}%", # 表に表示
                "買い目": ", ".join(bets) if bets else "見",
                "点数": len(bets), "結果": actual_result, "的中": hit_str,
        ...
        if matches:
            ...
            if bet_races:
                ...
                st.success(f"🎯 的中: {len(hits)}件 (的中率 {hit_rate:.1f}%)")
                st.info(f"💰 **総投資**: {total_invest:,}円 / **総回収**: {total_return:,}円 (回収率: {ret_rate:.1f}%)")
            else:
                st.warning("設定したフィルター条件を満たすレースがありませんでした。数値を下げてみてください。")
            ...
            if all_train_data:
                ...
                st.download_button(
                    ...
                    data=csv_data, file_name="kyotei_pure_ai_data.csv", mime="text/csv", type="primary", use_container_width=True
                )
